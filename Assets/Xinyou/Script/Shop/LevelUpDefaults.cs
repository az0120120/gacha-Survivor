...
public static class LevelUpDefaults
{
    public static ShopItemDefinition[] CreateRuntimeCatalog()
    {
        return new[]
        {
            CreateStat("lv_attack", "强攻", "攻击力 +15（所有武器）。", ItemEffectType.AttackUp, 1, 15f),
            CreateStat("lv_element", "元素之力", "元素攻击力 +5（所有武器）。", ItemEffectType.ElementalAttackUp, 1, 5f),
            CreateStat("lv_armor_pen", "破甲", "破防 +25（所有武器）。", ItemEffectType.ArmorPenetrationUp, 2, 25f),
            ...
            CreateStat("lv_crit_rate", "精准", "暴击率 +3%（所有武器）。", ItemEffectType.CritRateUp, 2, 3f),
            CreateStat("lv_crit_effect", "致命", "暴击效果 +10%（所有武器）。", ItemEffectType.CritEffectUp, 2, 10f),
